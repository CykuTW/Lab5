...
int turn_on_led(void) {
    ...
    fp = open_file("/sys/class/gpio/gpio165/direction", O_WRONLY, 0);
    write_file(fp, "out", 3);
    close_file(fp);

    fp = open_file("/sys/class/gpio/gpio165/value", O_WRONLY, 0);
    write_file(fp, "1", 1);
    close_file(fp);
...
static ssize_t drv_read(struct file *filp, char *buff, size_t count, loff_t *ppos) {
    ...
    switch_to_kernel_ds();
    fp = open_file("/sys/class/gpio/gpio165/value", O_RDONLY, 0);
    read_file(fp, value_buff, 1);
    close_file(fp);
